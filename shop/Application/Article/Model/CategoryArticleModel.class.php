...
class CategoryArticleModel extends Model {
    ...
    public function loadData($where = array(), $limit = 0 ,$order="A.sequence ASC , A.class_id ASC"){
        $pageList = $this->table("__CATEGORY__ as A")
                    ->join('LEFT JOIN __CATEGORY_ARTICLE__ as B ON A.class_id = B.class_id')
                    ->field('B.*,A.*')
                    ->where($where)
                    ->limit($limit)
                    ->order($order)
    ...
    public function saveData($type = 'add'){
        //事务总表处理
        $this->startTrans();
        $classId = D('Eshop/Category')->saveData($type);
        if(!$classId){
            $this->error = D('Eshop/Category')->getError();
            $this->rollback();
            return false;
        }
        //分表处理
    ...
    {
        //总表
        D('Eshop/Category')->delData($classId);
        //分表
        $map = array();
