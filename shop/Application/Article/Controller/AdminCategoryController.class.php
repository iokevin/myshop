...
class AdminCategoryController extends AdminController {
    ...
    public function add(){
        if(!IS_POST){
            $breadCrumb = array('栏目列表'=>U('Eshop/AdminCategory/index'),'文章栏目添加'=>U());
            $this->assign('breadCrumb',$breadCrumb);
            $this->assign('name','添加');
            $this->assign('categoryList',D('Eshop/Category')->loadList());
            $this->assign('tplList',D('Admin/Config')->tplList());
            $this->assign('expandList',D('Eshop/FieldsetExpand')->loadList());
    ...
    public function edit(){
        if(!IS_POST){
            ...
            $breadCrumb = array('栏目列表'=>U('Eshop/AdminCategory/index'),'文章栏目修改'=>U());
            $this->assign('breadCrumb',$breadCrumb);
            $this->assign('name','修改');
            $this->assign('categoryList',D('Eshop/Category')->loadList());
            $this->assign('tplList',D('Admin/Config')->tplList());
    ...
    public function del(){
        ...
        //判断子栏目
        if(D('Eshop/Category')->loadList('', $classId)){
            $this->error('请先删除子栏目！');
        }
        //判断栏目下内容
        $where = array();
