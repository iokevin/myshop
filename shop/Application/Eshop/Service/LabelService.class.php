...
class LabelService{
    ...
    public function contentList($data){
        ...
        if ($data['type']=='sub'&&!empty($data['class_id'])) {
            $classIds = D('Eshop/Category')->getSubClassId($data['class_id']);
        ...
        //其他属性
        $where['status'] = 1;
        $model = D('Eshop/Content');
        return D('Eshop/Content')->loadList($where,$data['limit'],$data['order']);
    }
    ...
    public function fragment($data){
        ...
        $info = D('Eshop/Fragment')->getWhereInfo($where);
        if(empty($info)){
            return ;
        }
    ...
    public function formToken($data){
        ...
        $formInfo = D('Eshop/FieldsetForm')->getWhereInfo($where);
        if(empty($formInfo)){
            return ;
        }
        return D('Eshop/FieldsetForm')->setToken($data['table']);
    }
    ...
    public function aurl($data){
        ...
        $info = D('Eshop/Content')->getWhereInfo($where);
        if(empty($info)){
            return ;
        }
        return D('Eshop/Content')->getUrl($info);
    }
    ...
    public function curl($data){
        ...
        $info = D('Eshop/Category')->getWhereInfo($where);
        if(empty($info)){
            return ;
        }
        return D('Eshop/Category')->getUrl($info);
    }
    ...
    public function tagsList($data){
        ...
        return D('Eshop/Tags')->loadList($where, $data['limit'], $data['order']);
    }
    ...
    public function formList($data)
    {
        ...
        $fieldList = D('Eshop/FieldForm')->loadList($where);
         //格式化表单内容为基本数据
        $data = array();
        ...
        if(!empty($list)){
            foreach ($list as $key => $value) {
                ...
                foreach ($fieldList as $v) {
                    $data[$key][$v['field']] = D('Eshop/FieldData')->revertField($value[$v['field']],$v['type'],$v['config']);
                }                
            }
        }
        return $data;
