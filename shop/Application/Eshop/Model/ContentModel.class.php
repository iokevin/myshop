...
class ContentModel extends Model {
    ...
    public function loadList($where = array(), $limit = 50, $order = 'A.time desc,A.content_id desc'){
        ...
        if(!empty($pageList)){
            ...
            foreach ($pageList as $key=>$value) {
                $list[$key]=$value;
                $list[$key]['aurl'] = D('Eshop/Content')->getUrl($value);
                $list[$key]['curl'] = D('Eshop/Category')->getUrl($value);
                $list[$key]['i'] = $i++;
            }
        }
        return $list;
    ...
    public function delData($contentId){
        ...
        //获取字段集信息
        $fieldsetInfo = D('Eshop/Fieldset')->getInfoClassId($info['class_id']);
        //删除扩展字段
        if(!empty($fieldsetInfo)){
            $expandModel = D('Eshop/FieldData');
    ...
    public function saveExtData($data){
        ...
        //获取字段集信息
        $fieldsetInfo = D('Eshop/Fieldset')->getInfoClassId($classId);
        //保存扩展字段
        if(!empty($fieldsetInfo)){
            $expandModel = D('Eshop/FieldData');
            //设置模型信息
    ...
    public function hasTags($keywords, $contentId){
        ...
        //设置模型
        $TagsHasModel = D('Eshop/TagsHas');
        $TagsModel = D('Eshop/Tags');
        //删除关联
        $where = array();
    ...
    public function getImage(){
        ...
        return D('Eshop/ContentTools')->getImage($content, $getImageNum);
    }
