    ...
    public function index(){
        ...
        if(!empty($list)){
            ...
            foreach ($list as $key => $value) {
                $data[$key]=$value;
                $data[$key]['curl']=D('Eshop/Category')->getUrl($value);
                $data[$key]['aurl']=D('Eshop/Content')->getUrl($value);

            }
        }
        //位置导航
        $crumb = array(
            array('name'=>'标签列表','url'=>U('Eshop/Tags/index')),
            array('name'=>$tagInfo['name'],'url'=>U('Eshop/TagsContent/index',array('name'=>$tagInfo['name']))),
            );
        //MEDIA信息
